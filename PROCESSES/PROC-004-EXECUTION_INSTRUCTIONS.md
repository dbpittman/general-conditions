...
- A component specification (PROC-003) is approved and baselined
- The component is complex enough that the agent's approach should be visible before execution begins
- The Division 01 submittals schedule requires an implementation plan for this component
...
1. Approved, baselined specification with acceptance criteria
...
4. Identify which steps touch existing systems, data, or code. These require the protections defined in GC-09 — backups, least privilege, user authorization for destructive operations
...
## Phase 2: Identify the Gaps in the Specification
...
## Phase 3: Present the Execution Plan
...
3. The plan should not re-specify the component. It references the specification; it doesn't replace it
...
2. The user is reviewing the approach, not the implementation details. GC-03.1: the user defines what, the agent determines how. The user should not need to approve individual code-level decisions unless the specification requires a specific approach
3. Clarification requests (PROC-005) identified in Phase 2 can proceed in parallel with plan approval

---

## Transition to Execution

Upon approval, the agent executes the plan, submitting intermediate outputs for review per PROC-006 at the defined gates. The execution plan, like the approach proposal, is a living document — but changes to the approved plan are documented and surfaced, not made silently.

During execution, the agent may invoke:
